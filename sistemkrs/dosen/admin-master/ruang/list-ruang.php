	...
	<div class="row">
	 <div class="col-sm-2">
	  <section class="panel panel-default">
	   <div class="panel panel-heading">Total List Ruang</div>
		<div class="panel panel-body">
			<p>Total List Ruang<b>:</b>
			<?php 
				echo mysqli_num_rows($select_ruang);
			?>
			</p>
		</div>
	 </div>
	 <div class="col-sm-10">
	  <section class="panel panel-default">
		<header class="panel-heading">LIST RUANG</header>
		 ...
		 <div class="table-responsive">
		  <table class="table table-bordered" id="example">
			...
			while($row_ruang = mysqli_fetch_array($select_ruang)){
			  echo '<tr>';
			  echo '<td>'.$no++.'</td>';
			  echo '<td>'.$row_ruang['nama_ruang'].'</td>';
			  echo "<td><a href='?hal=admin-master/ruang/edit-ruang&id_ruang=".$row_ruang['id_ruang']."' class='btn btn-warning' role='button'><i class='glyphicon glyphicon-pencil'></i></a>
                        <a href='#modal_delete' data-id='".$row_ruang['id_ruang']."' data-toggle='modal' class='btn btn-danger buang' role='button'><i class='glyphicon glyphicon-trash'></i></a>
					</td>";
			  echo '</tr>';
			 }
			?>
...
$('.buang').click(function(){
    var id=$(this).data('id');
    $('#delete_link').attr('href','?hal=admin-master/ruang/delete-ruang&id_ruang='+id);
})
</script>
